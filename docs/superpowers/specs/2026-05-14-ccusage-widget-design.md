...
- Default text: `$4.27 $(clippy)` — today's cost plus VS Code's built-in `search` codicon as the magnifying-glass affordance.
- Click: runs `ccusageWidget.refresh` (manual refresh; tooltip already auto-shows on hover, per VS Code conventions).
- States:
  - Loading (first poll not yet returned): `Loading… $(clippy)`
  - Stale (last poll failed but cache exists): keep last-known cost; tooltip explains staleness.
  - Hard failure (no cache yet): `$? $(clippy)` with retry link in tooltip.
...
|---|---|---|
| First poll in progress, no cache | `Loading… $(clippy)` | "Fetching ccusage data…" |
| All three calls failed, no prior cache | `$? $(clippy)` | Error message + `[Retry]` link + hint to set `ccusageCommand` |
...
| ccusage missing / spawn ENOENT | `$? $(clippy)` | Explicit "Could not run ccusage" message with config hint |
